...
public class Scenes {
	...
	// This scene builds a sheer like pyramid shape without editing the camera view nor lighting
	public static Scene scene5() {
		...
		// (2) We will also add spheres to form a sheer like pyramid shape. 
		for(int i= 0; i < 4; i++){
			for (int depth = 0; depth < 4; depth++) {
				for(int width=-1*depth; width<=depth; width++) {
					Shape sphereShape = new Sphere(new Point((double)width, (double)i, -1.0*(double)depth-i), 0.5);
					Material sphereMat = Material.getRandomMaterial();
					Surface sphereSurface = new Surface(sphereShape, sphereMat);
					finalScene.addSurface(sphereSurface);
				}
		...
		cutoffSpotlight.initIntensity(new Vec(0.6,0.6,1.0));
		finalScene.addLightSource(cutoffSpotlight);
		DirectionalLight directionalLight=new DirectionalLight(new Vec(0.5,-0.5,5.0),new Vec(0.7));
		finalScene.addLightSource(directionalLight);
		
		return finalScene;
	}
	
	// This scene builds a 5x5 cube and edits the camera view as well as lighting.
	public static Scene scene6() {
		// Define basic properties of the scene
		Scene finalScene = new Scene().initAmbient(new Vec(1.0))
				.initCamera(/* Camera Position = */new Point(3, 7, 15), 
						/* Towards Vector = */ new Vec(0.0, -0.1 ,-1.0),
						/* Up vector = */new Vec(0.0, 1.0, 0.0), 
						/*Distance to plain =*/ 2.0)
		...
		for(int i= 0; i < 5; i++){
			for (int depth = 0; depth < 5; depth++) {
				for(int width=0; width< 5; width++) {
					Shape sphereShape = new Sphere(new Point((double)width, (double)i, -1.0*(double)depth), 0.5);
					Material sphereMat = Material.getRandomMaterial();
					Surface sphereSurface = new Surface(sphereShape, sphereMat);
					finalScene.addSurface(sphereSurface);
				}
		...
		// Add light sources:
		CutoffSpotlight cutoffSpotlight = new CutoffSpotlight(new Vec(0.0, -1.0, 0.0), 90.0);
		cutoffSpotlight.initPosition(new Point(0.0, -30, 0.0));
		cutoffSpotlight.initIntensity(new Vec(1.0,0.6,0.6));
		finalScene.addLightSource(cutoffSpotlight);
		cutoffSpotlight = new CutoffSpotlight(new Vec(0.0, -1.0, 0.0), 90.0);
		cutoffSpotlight.initPosition(new Point(0, -30, 0.0));
		cutoffSpotlight.initIntensity(new Vec(0.6,1.0,0.6));
		finalScene.addLightSource(cutoffSpotlight);
		cutoffSpotlight = new CutoffSpotlight(new Vec(0.0, -1.0, 0.0), 180.0);
		cutoffSpotlight.initPosition(new Point(0.0, -30, 0.0));
		cutoffSpotlight.initIntensity(new Vec(0.6,0.6,1.0));
		finalScene.addLightSource(cutoffSpotlight);
		DirectionalLight directionalLight=new DirectionalLight(new Vec(0.5,-0.5,5.0),new Vec(2));
		finalScene.addLightSource(directionalLight);
		
		return finalScene;
	}
	
	// This scene builds a 5x5 square and edits the camera view as well as lighting.
		public static Scene scene7() {
			// Define basic properties of the scene
			Scene finalScene = new Scene().initAmbient(new Vec(1.0))
					.initCamera(/* Camera Position = */new Point(0, 2, 6), 
							/* Towards Vector = */ new Vec(0.0, -0.1 ,-1.0),
							/* Up vector = */new Vec(0.0, 1.0, 0.0), 
							/*Distance to plain =*/ 2.0)
			...
			for(int i = 0; i < 4; i++) {
				for (int depth = 0; depth < 4; depth++) {
					Shape sphereShape = new Sphere(new Point((double)depth, (double)i, -1.0), 0.4);
					Material sphereMat = Material.getRandomMaterial();
					Surface sphereSurface = new Surface(sphereShape, sphereMat);
					finalScene.addSurface(sphereSurface);
					}
			...
			// Add light sources:
			CutoffSpotlight cutoffSpotlight = new CutoffSpotlight(new Vec(0.0, -1.0, 0.0), 45.0);
			cutoffSpotlight.initPosition(new Point(0.0, 2.0, 6.0));
			cutoffSpotlight.initIntensity(new Vec(1.0,0.6,0.6));
			finalScene.addLightSource(cutoffSpotlight);
			cutoffSpotlight = new CutoffSpotlight(new Vec(0.0, -1.0, 0.0), 30.0);
			cutoffSpotlight.initPosition(new Point(-4.0, 4.0, -3.0));
			...
			cutoffSpotlight.initIntensity(new Vec(0.6,0.6,1.0));
			finalScene.addLightSource(cutoffSpotlight);
			DirectionalLight directionalLight=new DirectionalLight(new Vec(0,2,6.0),new Vec(0.7));
			finalScene.addLightSource(directionalLight);
			return finalScene;
		}
}
