...
public class MenuWindow extends JFrame implements Logger {
	...
	/**This method removes an object from an image using the Mask* */
	public void removeObjectFromImage(boolean[][] srcMask) {
		BufferedImage img;
		int outWidth = srcMask[0].length;
		// Calculating the delta
		int with3 = (srcMask[0].length / 3) -1;
		int maxRowSeamInRow = 0;
		for (int i = 0; i < srcMask.length; i++) {
			...
			if(numSeamsInRow > maxRowSeamInRow){
				maxRowSeamInRow = numSeamsInRow;
			}
		}
		int numOfSeamsToChange = Math.min(with3,maxRowSeamInRow);
		
		RGBWeights rgbWeights = colorMixer.getRGBWeights();
		scaleSelector.setWidth(scaleSelector.width() - numOfSeamsToChange);
		// Create a new Seam Carver Object where the cost matrix will be calculated
		SeamsCarver sc = new SeamsCarver(this, duplicateImage(), outWidth-numOfSeamsToChange, rgbWeights, srcMask);
		img = sc.resize();
		boolean[][] new_mask = sc.getMaskAfterSeamCarving();
		img = new SeamsCarver(this, (img), outWidth, rgbWeights, (new_mask)).resize();
